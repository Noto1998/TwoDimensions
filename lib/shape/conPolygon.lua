...
function ConPolygon:draw(mode)
    ...
    for i = 1, self.num do
        table[(i-1)*4+1] = _x+Base.getXYbyDir(radian+_dir*(i-1), self.len).x
        table[(i-1)*4+2] = _y+Base.getXYbyDir(radian+_dir*(i-1), self.len).y
        table[(i-1)*4+3] = _x+Base.getXYbyDir(radian+_dir*(i-1)+_dir/2, self.border).x
        table[(i-1)*4+4] = _y+Base.getXYbyDir(radian+_dir*(i-1)+_dir/2, self.border).y
    end
    -- fill
